...
<main class="mx-auto max-w-7xl px-5 py-12 lg:px-8">
  <div class="mb-8">
    <p class="eyebrow"><PackageOpen size={14}/> ADMIN</p>
    <h1 class="mt-2 text-3xl font-bold text-ink">จัดการของยืม</h1>
    <p class="mt-2 text-sm text-[#7c857e]">เพิ่มหรือลบรายการของได้จากหน้านี้</p>
  </div>
  <div class="grid gap-8 lg:grid-cols-[380px_1fr]">
    ...
    <div class="space-y-3">
      ...
      {/if}
      {#each items as item}
        <div class="flex items-center gap-4 rounded-2xl border border-[#e4e5de] bg-white p-4">
          <img src={item.image} alt="" class="h-16 w-16 rounded-xl object-cover"/>
          <div class="min-w-0 flex-1">
            <h3 class="font-bold text-ink">{item.name}</h3>
            <p class="mt-1 text-xs text-[#7c857e]">{item.category} · ว่าง {item.available}/{item.total} ชิ้น</p>
          </div>
          <button class="icon-btn text-red-500 disabled:cursor-not-allowed disabled:opacity-50" aria-label="ลบ {item.name}" disabled={deletingId === item.id} on:click={() => remove(item)}>
            <Trash2 size={17}/>
          </button>
        </div>
      {/each}
