        ...
        <div class="flex flex-col justify-between gap-4 md:flex-row md:items-start">
          <div>
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span class="rounded-full bg-[#eef2ed] px-3 py-1 text-xs font-bold text-sage">{statusLabel(request)}</span>
            </div>
            ...
            {#if request.status === 'rejected' && (request.teacherComment || request.adminComment)}
              <p class="mt-3 rounded-lg bg-[#fff1ef] px-3 py-2 text-sm text-[#a34e43]">
                <b>{request.teacherComment ? 'เหตุผลจากอาจารย์:' : 'เหตุผลจากผู้ดูแล:'}</b> {request.teacherComment || request.adminComment}
              </p>
            {/if}
          </div>
          {#if role === 'teachers' && request.status === 'pending_teacher'}
            <div class="flex gap-2">
              <Button variant="outline" size="sm" on:click={() => startReject(request)}><X size={15}/>ไม่รับทราบ</Button>
              <Button size="sm" on:click={() => onAction(request, 'pending_caretaker')}><Check size={15}/>รับทราบ</Button>
            </div>
          {:else if role === 'superadmin' && request.status === 'pending_caretaker'}
            <div class="flex gap-2">
              <Button variant="outline" size="sm" on:click={() => startReject(request)}><X size={15}/>ไม่อนุมัติ</Button>
              <Button size="sm" on:click={() => onAction(request, 'approved')}><Check size={15}/>อนุมัติ</Button>
            </div>
          {/if}
        </div>
        {#if rejectingId === request.id}
          <div class="mt-4 rounded-xl border border-[#f2cfc9] bg-[#fff8f6] p-4">
            <label>{role === 'teachers' ? 'เหตุผลที่ไม่รับทราบ' : 'เหตุผลที่ไม่อนุมัติ'}
              <textarea bind:value={rejectComment} rows="3" required placeholder="ระบุเหตุผลให้ผู้ขอยืมทราบ"></textarea>
            </label>
            ...
            <div class="mt-3 flex gap-2">
              ...
              <Button size="sm" on:click={() => confirmReject(request)}><X size={15}/>ยืนยันไม่อนุมัติ</Button>
            </div>
          </div>
        {/if}
      </article>
